# Horse Mod
The Horse mod is a Project Zomboid mod that adds horses to the game, gear to equip on them and the ability to ride them.

## Documentation
This mod provides a documentation for modders to create horse-related content, you can find it [here](https://pz-horseteam.github.io/horsemod/). Alternatively, you can build the documentation locally by following these steps:
```bash
cd docs
make html
```

An example mod is available at [PZ-HorseTeam/Example-mod](https://github.com/PZ-HorseTeam/Example-mod).